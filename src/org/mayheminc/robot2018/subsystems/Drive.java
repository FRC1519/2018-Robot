		...
		if (Robot.oi.autoTarget()) {
			...
			if (rawSteeringX == 0.0) { 
				...
				if (throttle == 0.0) {
					...
					// driving straight
					if ((m_iterationsSinceRotationCommanded == LOOPS_GYRO_DELAY) || (m_iterationsSinceMovementCommanded >=LOOPS_GYRO_DELAY)) {
						// exactly five iterations with no commanded turn, get current heading as desired heading
